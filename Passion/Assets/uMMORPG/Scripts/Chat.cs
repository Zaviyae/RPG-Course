...
public partial class Chat : NetworkBehaviour
{
    ...
    public override void OnStartLocalPlayer()
    {
        ...
        // test messages
        chat.AddMessage(new MessageInfo("", info.identifierIn, "Just type a message here to chat!", "", info.color));
        chat.AddMessage(new MessageInfo("", info.identifierIn, "  Use /g for guild chat", "",  info.color));
        chat.AddMessage(new MessageInfo("", info.identifierIn, "  Use /w NAME to whisper a player", "",  info.color));
        chat.AddMessage(new MessageInfo("", info.identifierIn, "  Or click on a message to reply", "",  info.color));
        chat.AddMessage(new MessageInfo("Someone", guild.identifierIn, "Anyone here?", "/g ",  guild.color));
        chat.AddMessage(new MessageInfo("Someone", party.identifierIn, "Let's hunt!", "/p ",  party.color));
        chat.AddMessage(new MessageInfo("Someone", whisper.identifierIn, "Are you there?", "/w Someone ",  whisper.color));
        chat.AddMessage(new MessageInfo("Someone", local.identifierIn, "Hello!", "",  local.color));
    ...
    public void TargetMsgWhisperFrom(NetworkConnection target, string sender, string message)
    {
        ...
        chat.AddMessage(new MessageInfo(sender, identifier, message, reply, whisper.color));
    }
    ...
    public void TargetMsgWhisperTo(NetworkConnection target, string receiver, string message)
    {
        ...
        chat.AddMessage(new MessageInfo(receiver, identifier, message, reply, whisper.color));
    }
    ...
    public void TargetMsgLocal(NetworkConnection target, string sender, string message)
    {
        ...
        string reply = whisper.command + " " + sender + " "; // whisper
        chat.AddMessage(new MessageInfo(sender, identifier, message, reply, local.color));
    }
    ...
    {
        string reply = whisper.command + " " + sender + " "; // whisper
        chat.AddMessage(new MessageInfo(sender, guild.identifierIn, message, reply, guild.color));
    }
    ...
    {
        string reply = whisper.command + " " + sender + " "; // whisper
        chat.AddMessage(new MessageInfo(sender, party.identifierIn, message, reply, party.color));
    }

    [TargetRpc(channel=Channels.DefaultUnreliable)] // only send to one client
    public void TargetMsgInfo(NetworkConnection target, string message)
    {
        chat.AddMessage(new MessageInfo("", info.identifierIn, message, "", info.color));
    }
}
