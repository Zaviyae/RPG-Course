...
public partial class Database
{
    ...
    static Database()
    {
        ...
        // note: 'guild' in here instead of extra table for faster & shorter queries
        ...
        // (100k players & 10 guild query: 180ms without index, 16 ms with index)
        ExecuteNonQuery("CREATE INDEX IF NOT EXISTS character_by_guild ON characters (guild)");
    ...
    static void LoadGuild(Player player)
    {
        ...
        {
            // load guild info
            List< List<object> > table = ExecuteReader("SELECT notice FROM guild_info WHERE name=@guild", new SqliteParameter("@guild", player.guildName));
            if (table.Count == 1)
            {
                List<object> row = table[0];
            ...
            table = ExecuteReader("SELECT name, level, guildRank FROM characters WHERE guild=@guild", new SqliteParameter("@guild", player.guildName));
            foreach (List<object> row in table)
            {
                GuildMember member = new GuildMember();
                member.name = (string)row[0];
                member.level = Convert.ToInt32((long)row[1]);
                member.rank = (GuildRank)Convert.ToInt32((long)row[2]);
    ...
    public static GameObject CharacterLoad(string characterName, List<Player> prefabs) {
        ...
        if (table.Count == 1)
        {
            ...
            if (prefab != null)
            {
                ...
                player.guildName          = (string)mainrow[15];
    ...
    public static void CharacterSave(Player player, bool online, bool useTransaction = true)
    {
        ...
        ExecuteNonQuery("INSERT OR REPLACE INTO characters VALUES (@name, @account, @class, @x, @y, @z, @level, @health, @mana, @strength, @intelligence, @experience, @skillExperience, @gold, @coins, @guild, @guildRank, @online, 0)",
                        ...
                        new SqliteParameter("@guild", player.guildName),
                        new SqliteParameter("@guildRank", player.InGuild() ? player.guild.members[player.guild.GetMemberIndex(player.name)].rank : GuildRank.Member),
